...
- **Scene hierarchy** (left sidebar) — Expand/collapse nodes, toggle visibility. The root node displays the model's `name` from `models.js`. Node labels are prettified for display (underscores → spaces, lowercase words capitalized, CAD duplicate suffixes like `(2)`/`003`/`v2` stripped); a sidecar `displayName` overrides this, and hovering a label shows the raw node name. The **Raw names** checkbox temporarily shows unmodified node names (`displayName` overrides included) — handy while authoring spec paths and `autoAssign` globs
